...
site_terms = ['Цена', u'Количество комнат', u'Общая площадь', u'Год постройки']
out_file = 'out.txt'
    ...
    for row in table.find_all("tr"):                  
        ...
        if (k != None):
            ...
            if ((v != None) and (v.div != None)):
                ktext = k.string
                vtext = v.div.string
                if ( (ktext in site_terms) and (is_number(vtext)) ):
#                    print('{} - {}'.format(ktext, vtext))
                    d[i][site_terms.index(ktext)] = vtext
...
#    try:
#        url = '2013-10-16/page{}.htm'.format(i)
#        s = open(url, 'r').read()
#    except IOError:
